...
export default function PeakFlowRateChart({ data = [] }) {
  const chartData = data.map((point, index) => {
    const flowRate = Number(point.flowRate ?? 0);

    return {
      time: point.time ?? `${index * 20}`,
      peakFlowRate: point.peakFlowRate ?? flowRate * 1.25,
    };
  });
